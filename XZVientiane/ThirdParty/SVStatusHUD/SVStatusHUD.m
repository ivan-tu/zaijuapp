...
- (void)dealloc {
	...
	if(fadeOutTimer != nil)
        (void)([fadeOutTimer invalidate]), (void)([fadeOutTimer release]), fadeOutTimer = nil;
...
+ (void)showWithMessage:(NSString*)message{
    [SVStatusHUD showWithImage:[UIImage imageNamed:@"qqwer@2x"] status:message];
}
...
- (void)setStatus:(NSString *)string {
    ...
    CGFloat hudWidth = 240;
    CGFloat hudHeight = 60;
	
	self.hudView.bounds = CGRectMake(0, 0, hudWidth, hudHeight);
	
    if(!string)
        self.imageView.center = CGPointMake(self.hudView.bounds.size.width/2, 40);
    else
        self.imageView.center = CGPointMake(self.hudView.bounds.size.width/2, 40);
...
//                             [overlayWindow release], overlayWindow = nil;
                             ...
                             //NSLog(@"在局%@", [UIApplication sharedApplication].windows);
                         }
                     }];
}
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        stringLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, 210, 40)];
...
- (SVStatusImage *)imageView {
    if (imageView == nil) {
        imageView = [[SVStatusImage alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
