...
- (void)setIsStatusBarDefault:(BOOL)isStatusBarDefault {
    ...
    if (isStatusBarDefault) {
        self.statusBarStyle = iOS8Later ? UIStatusBarStyleDefault : UIStatusBarStyleBlackOpaque;
    } else {
        self.statusBarStyle = iOS8Later ? UIStatusBarStyleLightContent : UIStatusBarStyleBlackOpaque;
    }
}
...
- (instancetype)initCropTypeWithAsset:(PHAsset *)asset photo:(UIImage *)photo completion:(void (^)(UIImage *cropImage,PHAsset * asset))completion {
    ...
    if (self) {
        ...
        __weak typeof(self) weakSelf = self;
        [previewVc setDoneButtonClickBlockCropMode:^(UIImage *cropImage, PHAsset * asset) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            [strongSelf dismissViewControllerAnimated:YES completion:^{
                if (completion) {
...
- (void)dealloc {
    // NSLog(@"在局%@ dealloc",NSStringFromClass(self.class));
}
...
- (void)willInterfaceOrientionChange {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (![UIApplication sharedApplication].statusBarHidden) {
            if (iOS8Later && self.needShowStatusBar) [UIApplication sharedApplication].statusBarHidden = NO;
        }
    });
}
...
- (void)configTableView {
    ...
    // 在局Claude Code[线程安全] 在主线程获取navigationController，避免后台线程访问UI
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
        ...
        [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<TZAlbumModel *> *models) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (self->_albumArr.count == 0) {
                    ...
                    noAlbumLabel.textColor = [UIColor grayColor];
                    noAlbumLabel.font = [UIFont systemFontOfSize:16];
                    [self.view addSubview:noAlbumLabel];
                }
            });
        }];
    });
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (iOS8Later && !isStatusBarHidden) top += [TZCommonTools tz_statusBarHeight];
