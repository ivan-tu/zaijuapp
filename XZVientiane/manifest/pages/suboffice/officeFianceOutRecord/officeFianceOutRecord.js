(function() {
	...
	app.Page({
		pageId: 'suboffice-officeFianceOutRecord',
		data: {
			systemId: 'suboffice',
			moduleId: 'officeFianceOutRecord',
			...
			form: {
				page:1,
				size:30,
				gettype:'platform',//=user/platform推广支持，服务支出
				clubid:'',
				timestap:'',
				begindate:'',
				enddate:'',
		...
		methods: {
			onLoad: function(options) {
				let _this = this;
				if(options.gettype=='platform'){
					app.setPageTitle('服务支出');
				}else if(options.gettype=='user'){
					app.setPageTitle('推广支出');
				};
				this.setData({
			...
			getList:function(loadMore){
				...
				app.request('//clubapi/getClubFinanceOutLog',formData,function(backData){
